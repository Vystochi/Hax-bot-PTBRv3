...
var countdownActive = false;
	...
	var warningSent = false;
     ...
     var isProrrogacao = false;
     var warningSent30 = false;
     var warningSent10 = false;
     var isProrrogacao = false;  // Renomeado de isOvertime
	 var currentMapLimits = { time: 6, goals: 5, prorrogacaoTime: 3 };  // Adicionado prorrogacaoTime
...
function updateStadium() {
    ...
    if (playerCount <= 4) {
        ...
        maxPlayersPerTeam = 2;
        maxTotalPlayers = 4;
        currentMapLimits = { time: 3, goals: 3, prorrogacaoTime: 3, totalTime: 6 };  // Stadiumx2: 3 min normais + 3 min prorrogação = 6 min total
    } else if (playerCount <= 9) {
        room.setCustomStadium(stadiumx3);
        maxPlayersPerTeam = 3;
        maxTotalPlayers = 6;
        currentMapLimits = { time: 5, goals: 5, prorrogacaoTime: 3, totalTime: 8 };  // Stadiumx3: 5 min normais + 3 min prorrogação = 8 min total
    } else {
        room.setCustomStadium(stadiumx6);
        maxPlayersPerTeam = 6;
        maxTotalPlayers = Infinity;
        currentMapLimits = { time: 10, goals: 8, prorrogacaoTime: 3, totalTime: 13 };  // Stadiumx6: 10 min normais + 3 min prorrogação = 13 min total
    }
    
    room.setTimeLimit(currentMapLimits.totalTime);  // Define tempo total (normais + prorrogação)
...
function shuffleArray(array) {
	...
	function assignTeams() {
	    try {
	        ...
	        // Só redistribui se houver espectadores, desequilíbrio (>1 diferença) ou se for inicialização forçada
	        if (spectators.length === 0 && Math.abs(team1Count - team2Count) <= 1 && activePlayers <= maxTotalPlayers) {
	            console.log("assignTeams: Times já balanceados, não redistribuindo.");
	            return;
	        }
	        
	        // Embaralha apenas espectadores ou rebalanceia se desequilibrado
	        if (spectators.length > 0) {
	            ...
	            // Rebalanceia times existentes se desequilibrados
	...
	room.onPlayerJoin = function (player) {
	    ...
	    updateAdmins();
	    updateStadium();  // Atualiza limites
	    // Não chama assignTeams() aqui se jogo estiver rodando; será chamado periodicamente em onGameTick ou após vitória
	    var players = room.getPlayerList();
	    if (players.length < 7) {
...
room.onGameTick = function () {
    try {
        ...
        // Avisos antecipados
        if (scores.time >= (timeLimitSeconds - warningTime30) && scores.time < timeLimitSeconds && scores.red === scores.blue && !warningSent30) {
            warningSent30 = true;
            room.sendAnnouncement(centerText(`⚠️ EMPATE IMINENTE! Fim em 30 segundos...`), null, announcementColor, "bold", Notification.CHAT);
        }
        if (scores.time >= (timeLimitSeconds - warningTime10) && scores.time < timeLimitSeconds && scores.red === scores.blue && !warningSent10) {
            warningSent10 = true;
            room.sendAnnouncement(centerText(`⚠️ EMPATE IMINENTE! Fim em 10 segundos...`), null, announcementColor, "bold", Notification.CHAT);
        }
        
        // Contagem regressiva (3, 2, 1)
        if (scores.time >= (timeLimitSeconds - 3) && scores.time < timeLimitSeconds && scores.red === scores.blue && !countdownActive) {
        ...
        if (scores.time >= prorrogacaoStart && scores.time < timeLimitSeconds && scores.red === scores.blue && !isProrrogacao) {
            isProrrogacao = true;
            room.sendAnnouncement(centerText(`⏰ E LA VAMOS PARA A PRORROGAÇÃO - ${currentMapLimits.prorrogacaoTime} min`), null, announcementColor, "bold", Notification.CHAT);
        }
        
        // Fim total: Reinicia se empatado (sem pênaltis)
